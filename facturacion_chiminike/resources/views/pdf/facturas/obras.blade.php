...
@php
  ...
  // === Contar entradas vendidas (detecta por descripción, código o tipo_factura) ===
  $entradasVendidas = 0;
  if (!empty($detalles) && is_iterable($detalles)) {
    foreach ($detalles as $d) {
      $desc = strtolower((string)($d['descripcion'] ?? ''));
      $cod  = strtolower((string)($d['codigo'] ?? ''));
      $cant = (int)($d['cantidad'] ?? 0);

      $esEntrada = (
        preg_match('/entrada|boleto|boleter|teatro|obra/i', $desc) ||
        preg_match('/^(entr|bol)/i', $cod) ||
        preg_match('/obra|teatral|boleter/i', $tipoBoleteria)
      );
...
<div class="sheet">
  ...
  <div class="copy emisor">
    <div class="page">
      ...
      <table class="client-head">
        ...
        <tr>
          <td class="label">&nbsp;</td>
           <td class="line">Boletería/ Obra Teatral</td>
        </tr>
        <tr>
  ...
  @php
    ...
    // SOLO detecta entradas por descripción o código, NO por tipo_factura
    $esEntrada = (
      preg_match('/\b(entrada|boleto|boleta|ticket)s?\b/i', $descRaw) ||
      preg_match('/^(entr|bol|tick)/i', $cod)
    );
  @endphp

  @if($esEntrada && $cant > 0)
    Compra de {{ $cant }} {{ $cant === 1 ? 'boletos' : 'boletos' }} para obra teatral
    {{-- si quieres "hora teatral", cambia “obra” por “hora” aquí --}}
  @else
    {{ hn_sin_precio($descRaw) }}
  @endif
</td>
