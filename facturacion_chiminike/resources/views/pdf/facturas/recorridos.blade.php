...
@php
  ...
  if (!function_exists('hn_numero_en_letras')) {
    function hn_numero_en_letras($num) {
      ...
      if ($resto > 0) {
        $base = $tres($resto);
        $base = preg_replace('/\buno$/','uno', $base);
  ...
  /* =======================
   NUEVO: combinar Entradas + Paquetes
   - Descripción con nombres reales
   - Cantidad y Total sumados
   - Precio unitario = (precio unit Entradas) + (precio unit Paquetes)
     * Si hay varias entradas con distinto precio: promedia el grupo de entradas.
     * Si hay varios paquetes con distinto precio: promedia el grupo de paquetes.
=======================*/
$E_qty = 0; $E_total = 0; $E_names = []; $E_codes = [];
$P_qty = 0; $P_total = 0; $P_names = []; $P_codes = [];
$otros = [];

foreach ($detalles as $d) {
  $descRaw   = $d['descripcion'] ?? '';
  $descMin   = mb_strtolower($descRaw, 'UTF-8');
  $codeMin   = mb_strtolower($d['codigo'] ?? '', 'UTF-8');
  $descClean = hn_sin_precio($descRaw);

  // === ENTRADAS **o** TOUR POR SALAS INTERACTIVAS ===
  $esEntradaOTour =
        preg_match('/\bentrada(s)?\b/u', $descMin)                               // entradas
     || ($codeMin === 'turporsalinteractiva')                                    // código
     || preg_match('/\btour\s+por\s+salas?\s+interactivas?\b/u', $descMin);      // descripción

  if ($esEntradaOTour) {
    $E_qty   += (float)($d['cantidad'] ?? 0);
    $E_total += (float)($d['total'] ?? 0);
    if ($descClean) $E_names[$descClean] = true;
    if (!empty($d['codigo'])) $E_codes[$d['codigo']] = true;
    continue;
  }

  // === PAQUETES ===
  if (preg_match('/\bpaquete(s)?\b/u', $descMin)) {
    $P_qty   += (float)($d['cantidad'] ?? 0);
    $P_total += (float)($d['total'] ?? 0);
    if ($descClean) $P_names[$descClean] = true;
    if (!empty($d['codigo'])) $P_codes[$d['codigo']] = true;
    continue;
  }
...
$comboQty = $E_qty + $P_qty;           // Cantidad sumada
$comboTot = $E_total + $P_total;       // Total sumado

if ($comboQty > 0) {
  // Precio unitario de cada grupo (si falta uno, vale 0)
  $E_unit = $E_qty > 0 ? ($E_total / max($E_qty, 1)) : 0.0;
  $P_unit = $P_qty > 0 ? ($P_total / max($P_qty, 1)) : 0.0;

  // *** Suma de precios unitarios ***
  $comboUnit = $E_unit + $P_unit;

  // Descripción con nombres reales
  $partesDesc = [];
  if ($E_qty > 0) { $partesDesc[] = ' '.implode(', ', array_keys($E_names)); }
  if ($P_qty > 0) { $partesDesc[] = ' '.implode(', ', array_keys($P_names)); }
  $comboDesc = implode(' / ', $partesDesc);

  $comboCodes = array_merge(array_keys($E_codes), array_keys($P_codes));

  $filas[] = [
    'cantidad'        => $comboQty,
    'codigo'          => implode(',', $comboCodes),
    'descripcion'     => $comboDesc ?: 'Entradas/Tour por salas interactivas y paquetes',
    'precio_unitario' => $comboUnit,   // se mostrará con 2 decimales
    'total'           => $comboTot,
  ];
}

foreach ($otros as $d) { $filas[] = $d; }

$conteoFilas = count($filas);
  ...
  <div class="copy emisor">
    <div class="page">
      ...
      <table class="client-head">
        ...
        <tr>
          <td class="label">&nbsp;</td>
          <td class="line">Boletería / {{ $factura['tipo_factura'] ?? '—' }}</td>
        </tr>
        <tr>
      ...
      <table class="items">
        ...
        <tbody>
          ...
          <tr>
            ...
            <td class="txr">
              @if(isset($d['precio_unitario']) && $d['precio_unitario'] !== null)
                L. {{ number_format($d['precio_unitario'],2) }}
              @else
                L. 0.00
              @endif
            </td>
            <td class="txr">L. {{ number_format($d['total'] ?? 0,2) }}</td>
          </tr>
          @endforeach
          ...
          <tr>
            ...
            <td class="txl">{{ $cliente['nombre'] ?? '—' }}</td>
          ...
          @for($i = $conteoFilas + ($isEvento ? 1 : 2); $i < $minRows; $i++)
