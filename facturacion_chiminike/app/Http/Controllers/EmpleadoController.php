...
class EmpleadoController extends Controller
{
    public function index()
    {
        try {
            ...
            // Si algo falla, mejor evitar que explote la vista
    ...
    public function update(Request $request, $id)
    {
        ...
        $response = Http::put("http://localhost:3000/empleados/$id", $data);
    ...
    public function store(Request $request)
    {
        ...
        $nombre = Str::slug($request->nombre_persona, '.');
        $sufijo = substr($request->dni, -3);
        $nombre_usuario = strtolower($nombre . $sufijo);


        $contrasena = Str::random(10);


        $creado_por = session('usuario.nombre_usuario') ?? 'sistema';
